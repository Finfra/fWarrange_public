...
## 4.8 RestoreStats (Issue72_1 Phase 1)

창 복구 매칭의 누적 통계. `POST /layouts/{name}/restore` 호출 시마다 `WindowRestoreService.computeMatchScore` 결과를 `RestoreStatsCollector`가 수집하여 영속함.

| Method | Path               | 설명                                  |
| ------ | ------------------ | ------------------------------------- |
| GET    | `/restore-stats`   | 누적 통계 스냅샷                      |
| DELETE | `/restore-stats`   | 통계 초기화 (베이스라인 재시작용)     |

* **영속 위치**: `~/Library/Application Support/fWarrangeCli/restore-stats.json` (env `fWarrangeCli_stats_path`로 재정의)
