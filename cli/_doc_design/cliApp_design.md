...
* 기존 REST API 스펙(`api/openapi.yaml`) 최대한 유지 — 외부 연동 호환성 보존
...
기존 API 스펙 유지 — `openapi.yaml` 변경 최소화:
...
| PUT    | `/api/v1/layouts/{name}`            | 레이아웃 저장     |
| DELETE | `/api/v1/layouts/{name}`            | 레이아웃 삭제     |
| POST   | `/api/v1/capture`                   | 현재 창 캡처      |
...
| GET    | `/api/v1/locale`                    | 현재 로케일       |
| PUT    | `/api/v1/locale`                    | 로케일 변경       |
...
| Method | Path                   | 설명                          |
| :----- | :--------------------- | :---------------------------- |
| GET    | `/api/v1/cli/status`  | cliApp 상태 정보              |
| GET    | `/api/v1/cli/version` | cliApp 버전                   |
| POST   | `/api/v1/cli/quit`    | cliApp 종료 (확인 필요)       |
...
| 버전 표시          | `GET /api/v1/cli/version` 응답값 표시                   |
