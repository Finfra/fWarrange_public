#!/bin/bash
# cmdTestDo.sh - CLI 커맨드 테스트 스크립트 실행기
# Usage:
#   source cli/_tool/cmdTestDo.sh        # 정상 테스트 전체 실행
#   source cli/_tool/cmdTestDo.sh 0      # 0번만 실행
#   source cli/_tool/cmdTestDo.sh E      # 에러 테스트 전체 실행
#   source cli/_tool/cmdTestDo.sh E01    # E01번만 실행
#   source cli/_tool/cmdTestDo.sh all    # 정상 + 에러 전체 실행
...
SCRIPT_DIR="$(cd "$(dirname "$_SELF")" && pwd)/cmdTest"
