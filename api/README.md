---
title: fWarrange REST API Documentation
description: fWarrange REST API 사용 방법 및 엔드포인트 문서 (English)
date: 2026-03-26
---
...
Controls the app UI state for capture automation. Sends a notification to the app to hide/show windows or select/deselect apps in the layout detail view.

### Request Parameters

| Field         | Type                  | Required | Description                                                             |
| ------------- | --------------------- | -------- | ----------------------------------------------------------------------- |
| `hideWindows` | boolean               | No       | Hide or show the window list in layout detail view                      |
| `selectApps`  | string or string[]    | No       | `"all"`, `"none"`, `"top:N"`, or array of app names                    |
| `excludeApps` | string[]              | No       | Apps to exclude from selection (used with `selectApps`)                 |
...
# Set UI state (hide windows)
curl -X PUT http://localhost:3016/api/v1/ui/state \
  -H "Content-Type: application/json" \
  -d '{"hideWindows": true}'

# Set UI state (select specific apps)
curl -X PUT http://localhost:3016/api/v1/ui/state \
  -H "Content-Type: application/json" \
  -d '{"selectApps": ["Safari", "iTerm2"]}'
