...
server.tool(
  "set_ui_state",
  "앱 UI 상태를 제어합니다 (캡처 자동화용: 창 목록 숨기기, 앱 선택 등)",
  {
    hideWindows: z
      .boolean()
      .optional()
      .describe("창 목록 숨기기/표시"),
    selectApps: z
      .union([z.string(), z.array(z.string())])
      .optional()
      .describe(
        '앱 선택 모드: "all", "none", "top:N", 또는 앱 이름 배열'
      ),
    excludeApps: z
      .array(z.string())
      .optional()
      .describe("선택에서 제외할 앱 목록"),
  },
  withErrorHandler(async ({ hideWindows, selectApps, excludeApps }) => {
    const body = {};
    if (hideWindows !== undefined) body.hideWindows = hideWindows;
