...
#   bash cli/_tool/apiTestDo.sh v2 15                    # v2 15번
...
LOG_FILE="$HOME/Documents/finfra/fWarrangeData/logs/wlog_cliApp.log"
...
case "${1:-}" in
    ...
    all)
        ...
        [ "$OPT_REPORT" -eq 1 ] && save_report "apiTest"
...
[ "$OPT_LOG" -eq 1 ] && check_logs
[ "$OPT_REPORT" -eq 1 ] && save_report "apiTest_${VERSION}"
