---
name: old_issue
description: Issue.md ✅ 완료 섹션에서 이관된 종결 이슈 아카이브
date: 2026-05-02
---
...
* 상세:
    ...
    - 방식 A (구현됨, commit: ed2a225): `AppState.setLaunchAtLogin()` — `true`면 `brew services start`, `false`면 `launchctl bootout` + plist 직접 rm
    - 방식 B (구현됨, commit: f8c6063): Homebrew formula `keep_alive = false` 명시로 기본값 제거 (Homebrew 자동 재시작 우회)
...
* **설계 한계와 우회 방법**:
    - Homebrew 설계상 `brew services stop`은 plist를 완전히 제거함 — 앱 차원 제어 불가
    - 사용자 교육: 터미널 대신 **메뉴바 토글 버튼** 사용 권장
    - 메뉴바 토글로 `launchAtLogin` 제어 시 plist 상태와 설정 완벽 동기화
* 커밋: ed2a225 (핵심 구현), f8c6063 (Homebrew 기본값 제거)
